...
import { setGameState, setMovesState, clearGameState, clearUserMoves, stepBack} from '../../../store/reducers/GroupSlice';
...
const ChessBoard: FC<ChessBoardProps> = ({moveMode, globalMode, clear, setClear, position, materialId, movesM, setMoves, PrevBackTheme, type, custom, game, setGame}) => {
        ...
        const updateHistory = async () => {
            if(groupId) {
                if(game.history() && user.role !== 'STUDENT') {
                    await dispatch(editGroup({groupId: groupId, payload: {current: materialId}}));
                }
                if(user.role !== 'TRANER' && user.role !== 'DIRECTOR' && user.role !== 'ZDIRECTOR') {
                    await dispatch(editGroup({groupId: groupId, payload: {material: materialId, move: {user_id: user._id, name: user.name, sname: user.sname, moves: []}}}))
    ...
    async function onSquareClick(square: Square) {
        if(moveMode || user.role === 'DIRECTOR' || user.role === 'ZDIRECTOR' || user.role === 'TRANER') {
        ...
        // to square
            ...
            // if promotion move
                ...
                if(move) {
                    ...
                    if(groupId) {
                        await GroupService.editGroup(groupId, {open: true, material: materialId, move: {user_id: user._id, name: user.name, sname: user.sname, moves: [{color: move.color, move: move.san}]}});
