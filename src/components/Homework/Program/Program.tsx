            ...
            <div className="border-2 border-[#CCC] -mt-6 pt-6 rounded-b-2xl border-t-0">
                <div className={["flex flex-col overflow-auto", homework ? 'h-[calc(100vh-300px)]': 'max-h-[250px] max-2xl:max-h-[150px]'].join(' ')}>
                    {program.length > 0 &&
                        program.map((item, id)=>
                            <>
                                <Item status={HM.results.find(item=> item.user_id === user._id)?.results.find(itm=> itm.material === item._id)?.result} active={active} onClick={()=> setMaterialHandler(item)} key={item._id} id={item._id} type={item.type} number={id + 1}/>
                            </>     
                        )
                    }
