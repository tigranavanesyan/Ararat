...
const Item: FC<ItemProps> = ({id, type, number, onClick, active , status}) => {
    ...
    return (
        <div className="flex items-center px-4 font-medium text-xl max-2xl:text-base border-b-2 border-b-[#CCC] last:border-b-0 py-1">
            <button onClick={onClick} className={['w-full flex justify-start ', active === id && 'text-apricot'].join(' ')}><span className='mr-5 text-red-500 font-semibold'>{number}</span>Задача {number} <div className={[status === 'passed' && 'bg-green-500 w-3 h-3 rounded-full mt-2 ml-5', status === 'failed' && 'bg-red-500 w-3 h-3 rounded-full mt-2 ml-5'].join(' ')}></div></button>
            {user.role !== 'STUDENT' &&
